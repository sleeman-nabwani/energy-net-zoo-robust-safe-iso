...
def penalty_spread(spread: float, soft_start: float, full: float | None = None) -> float:
    """
    ...
    Args:
        ...
        full:        optional threshold where penalty saturates (>= soft_start).
    """
    ...
    if full <= soft_start:
        raise ValueError(f"penalty_spread requires full>soft_start (got soft_start={soft_start}, full={full}).")
    return clip01((spread - soft_start) / max(full - soft_start, 1e-6))
